...
function generateQR() {
    ...
    new QRCode(qrContainer, { text: text, width: 256, height: 256 });
}
...
function saveGoal() {
    ...
    goals.push({ id: Date.now(), goal: goal, created: new Date().toLocaleString() });
...
function loadGoals() {
    ...
    goals.forEach(g => { html += `<li style="padding:10px; margin:5px 0; background:#f0f0f0; border-radius:6px;"><strong>${g.goal}</strong><br><small>${g.created}</small></li>`; });
    html += '</ul>';
...
function addProgress() {
    ...
    if (!progress.trim()) { alert('Please enter progress'); return; }
    let list = JSON.parse(localStorage.getItem('bp_progress') || '[]');
    list.push({ id: Date.now(), progress: progress, date: new Date().toLocaleString() });
...
function loadProgress() {
    ...
    list.reverse().forEach(p => { html += `<li style="padding:10px; margin:5px 0; background:#f0f0f0; border-radius:6px;"><strong>${p.progress}</strong><br><small>${p.date}</small></li>`; });
    html += '</ul>';
